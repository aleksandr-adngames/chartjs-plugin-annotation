...
module.exports = function(Chart) {
	...
	var LineAnnotation = Chart.Annotation.Element.extend({
		...
		configure: function() {
            ...
            var elements = this.chartInstance.chart.config.data.datasets[0].data;
            // var tooltipYPosition = this.chartInstance.chart.annotation.value;
			var elementsY = elements.map(function(item) { return item.y; }, this);
            if(activeElement){
                options.value = activeElement.x;
                console.log('all Y elements', elementsY);
                console.log('active elements', activeElement.y);
            }
            ...
            if(subLabelEnabled || true) {
				...
				model.subLabelXPadding = 10;
				model.subLabelYPadding = options.subLabel.yPadding;
				model.subLabelCornerRadius = 2;
			...
			model.labelFontSize = 16;
			model.labelFontStyle = options.label.fontStyle;
			model.labelFontColor = options.label.color;
			model.labelXPadding = 16;
			model.labelYPadding = 7;
			model.labelCornerRadius = 2;
			...
			model.labelY = (labelPosition.y - model.labelYPadding)+60;
            ...
            // // Figure out the sublabelCircle:
            // model.circleBackgroundColor = options.circle.backgroundColor;
            // model.circleFontFamily = options.circle.fontFamily;
            // model.circleFontSize = options.circle.fontSize;
            // model.circleFontStyle = options.circle.fontStyle;
            // model.circleFontColor = options.circle.color;
            // model.circleXPadding = options.circle.xPadding;
            // model.circleYPadding = options.circle.yPadding;
			//
			//
            // ctx.font = chartHelpers.fontString(model.circleFontSize, model.circleFontStyle, model.circleFontFamily);
		},
		...
		draw: function() {
			...
			if (view.labelEnabled && view.labelContent) {
                ...
                ctx.shadowColor = 'rgba(37, 109, 82, 0.32)';
                ctx.shadowBlur = 12;
                ctx.shadowOffsetX = 0;
                ctx.shadowOffsetY = 2;
			...
			if (view.sublLabelEnabled || false ){
                view.labelY -= 60;
			}
			//Draw SUBLABEL
			if (view.subLabelEnabled || true) {
				...
				ctx.fillText(
					...
					view.subLabelY-30 + (view.subLabelHeight / 2) - view.sublLabelLineHeight * 0.7
				);
				...
				ctx.fillText(
					view.sublLabelLabel,
					view.subLabelX + (view.subLabelWidth / 2),
					view.subLabelY-30 + (view.subLabelHeight / 2) + view.sublLabelLineHeight  * 1.2
				);
			}
            //draw Circle
            if (view.subLabelEnabled ) {
                var context = ctx;
                var centerX = view.x1;
                var centerY = view.subLabelY-30;
                var radius = 8;

                context.restore();
                context.beginPath();
                context.arc(centerX, centerY, radius, 0, 2 * Math.PI, false);
                // context.moveTo(view.x2-view.x1, view.y1);
                context.closePath();
                context.fillStyle = 'rgb(247, 97, 32)';
                context.fill();

                var circleFont = view.subLabelFontFamily;

                context.font = 'normal 12px '+ circleFont;
